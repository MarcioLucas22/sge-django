...
def get_sales_metrics() -> dict:
      ...
      total_products_sold = Outflow.objects.aggregate(total_products_sold=Sum('quantity'))['total_products_sold'] or 0 # Se nao houver vendas, retorna 0
...
def get_daily_sales_data() -> dict:
      ...
      return dict(
            dates=[datetime.strptime(date, '%Y-%m-%d').strftime('%d/%m/%Y') for date in dates],
            values=values
      )
